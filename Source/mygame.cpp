...
void CGameStateInit::OnInit()
{
	...
	press_space.AddBitmap("res/opening_menu/press_space.bmp");
	press_space.AddBitmap("res/opening_menu/press_space_blank.bmp", RGB(255, 255, 255));
...
{
	//title_logo.SetTopLeft((SIZE_X - logo.Width()) / 2, SIZE_Y / 8);
...
{
	title_logo.LoadBitmap("RES/opening_menu/title_logo.bmp");
	cursor.LoadBitmap("RES/opening_menu/cursor.bmp");
	hint.LoadBitmap("RES/opening_menu/saved_automatically.bmp");
	frame_action.LoadBitmap("RES/opening_menu/frame.bmp", RGB(255, 255, 255));
	record.LoadBitmap("RES/opening_menu/record.bmp", RGB(255, 255, 255));

	char *path_action[ACTION_AMOUNT] = {
	...
	for (int i = 0; i < ACTION_AMOUNT; i++) {
		...
		action[i].figure.LoadBitmap(path_action[i]);
	}
	action[CONTINUE].isSelected = true;
	moveCursor_action();
	

	frame_action.SetTopLeft((SIZE_X - title_logo.Width()) / 2 - 30, SIZE_Y / 2 - 15);
	title_logo.SetTopLeft((SIZE_X - title_logo.Width()) / 2, SIZE_Y / 10);
	record.SetTopLeft((SIZE_X - title_logo.Width()) + 220, SIZE_Y / 2 - 15);
	hint.SetTopLeft((SIZE_X - title_logo.Width()) + 150, SIZE_Y / 2 + 250);
	


	frame_quit.AddBitmap("RES/opening_menu/quit/frame.bmp", RGB(255, 255, 255));
	//frame_quit.AddBitmap("RES/opening_menu/quit/frame_blank.bmp", RGB(255, 255, 255));
	title_quit.AddBitmap("RES/opening_menu/quit/are_you_sure.bmp");
	//title_quit.AddBitmap("RES/opening_menu/quit/are_you_sure_blank.bmp", RGB(255, 255, 255));
	//char *path_quit_blank[QUIT_AMOUNT] = {
	//	"RES/opening_menu/quit/no_blank.bmp",
	//	"RES/opening_menu/quit/yes_blank.bmp",
	//};
	char *path_quit[QUIT_AMOUNT] = {
		"RES/opening_menu/quit/no.bmp",
		"RES/opening_menu/quit/yes.bmp",
	};
	for (int i = 0; i < QUIT_AMOUNT; i++) {
		quit[i].isSelected = false;
		//quit[i].figure.AddBitmap(path_quit_blank[i]);
		//quit[i].figure.AddBitmap(path_quit[i]);
		quit[i].figure.LoadBitmap(path_quit[i]);
	}
	quit[NO].isSelected = true;
	moveCursor_quit();
...
void CGameStateMenu::moveCursor_action() {
	for (int i = 0; i < ACTION_AMOUNT; i++) {
		if (action[i].isSelected) {
			cursor.SetTopLeft((SIZE_X - title_logo.Width()) / 2 - 15, SIZE_Y / 2 + 25 + 45 * i);
			action[i].figure.SetTopLeft((SIZE_X - title_logo.Width()) / 2 + 10, SIZE_Y / 2 + 25 + 45 * i);
		}
		else {
...
void CGameStateMenu::moveCursor_quit() {
	for (int i = 0; i < QUIT_AMOUNT; i++) {
		if (quit[i].isSelected) {
			cursor.SetTopLeft((SIZE_X - title_logo.Width()) / 2 - 15, SIZE_Y / 2 + 25 + 45 * i);
			quit[i].figure.SetTopLeft((SIZE_X - title_logo.Width()) / 2 + 10, SIZE_Y / 2 + 25 + 45 * i);
		}
		else {
			quit[i].figure.SetTopLeft((SIZE_X - title_logo.Width()) / 2 - 5, SIZE_Y / 2 + 25 + 45 * i);
		}
	}
}
...
{
	title_logo.ShowBitmap();
	frame_action.ShowBitmap();
	cursor.ShowBitmap();
	record.ShowBitmap();
	hint.ShowBitmap();
	int i = 0;
	for (i = 0; i < ACTION_AMOUNT; i++)
		action[i].figure.ShowBitmap();
	for (i = 0; i < QUIT_AMOUNT; i++)
		quit[i].figure.ShowBitmap();
}
